...
export function ExercisePlayer({ exercises, onAttempt, onFinish }: Props) {
  ...
  if (done) {
    ...
    return (
      <div className="animate-pop card text-center">
  ...
  return (
    <div>
      <div className="mb-3 flex items-center gap-3">
        ...
        <span className="text-xs font-semibold text-[var(--color-muted)]">{i + 1}/{exercises.length}</span>
      </div>
...
function ExerciseCard({ ex, onResult, onNext }: { ex: Exercise; onResult: (c: boolean, a: string) => void; onNext: () => void }) {
  ...
  function evaluate() {
    ...
    switch (ex.type) {
      ...
      case 'match':
        ...
        break;
      case 'freeform':
        ans = given; ok = true; break; // self-graded
    }
    setCorrect(ok);
    setChecked(true);
    if (ex.type === 'freeform') return; // result recorded on self-grade
    onResult(ok, ans);
  ...
  const canCheck = (() => {
    switch (ex.type) {
      ...
      case 'freeform': return given.trim().length > 0;
  ...
  return (
    <div className="animate-slideup card">
      {/* prompt */}
      {ex.type === 'listen' ? (
        <div className="flex flex-col items-center gap-3 py-2">
          <button onClick={() => speak(ex.audioText || '')} className="grid h-20 w-20 place-items-center rounded-full bg-[var(--color-surface2)] text-4xl text-[var(--color-sky)] active:scale-90">🔊</button>
          <p className="text-sm text-[var(--color-muted)]">Послушай и запиши предложение</p>
        </div>
      ) : ex.prompt ? (
        <div className="flex items-start gap-2">
          <p className="display flex-1 text-lg font-semibold leading-snug">{ex.prompt}</p>
          {promptIsEnglish && <SpeakButton text={ex.prompt.replace(/_+/g, ' ').replace(/\([^)]*\)/g, '')} />}
        </div>
      ) : null}

      {ex.hint && !checked && <p className="mt-1 text-xs text-[var(--color-muted)]">💡 {ex.hint}</p>}
            ...
            {ex.options?.map((opt) => {
              ...
              return (
                <button key={opt} disabled={checked} onClick={() => setSelected(opt)} className={`btn ${cls} justify-between !font-semibold`}>
                  ...
                  {checked && isSel && !isAnswer && <span>❌</span>}
                </button>
              );
            })}
          </div>
            ...
            {ex.pairs?.map((p) => {
              ...
              return (
                <div key={p.left} className="flex items-center gap-2">
                  <span className="display w-20 shrink-0 font-semibold">{p.left}</span>
                  <select
                    className="input flex-1"
                    value={matchSel[p.left] || ''}
                    disabled={checked}
      ...
      {checked && ex.type !== 'freeform' && (
        <div className={`animate-pop mt-4 rounded-2xl p-3 ${correct ? 'bg-[color-mix(in_srgb,var(--color-success)_14%,transparent)]' : 'bg-[color-mix(in_srgb,var(--color-danger)_14%,transparent)]'}`}>
          <div className="flex items-center gap-2 font-semibold">
            {correct ? <span className="text-[var(--color-success)]">✅ Верно!</span> : <span className="text-[var(--color-danger)]">❌ Не совсем</span>}
          </div>
          {!correct && (
            <div className="mt-1 flex items-center gap-2 text-sm">
              <span className="text-[var(--color-muted)]">Правильно:</span>
              <b className="text-[var(--color-text)]">{correctText}</b>
              <SpeakButton text={correctText} className="!h-7 !w-7 !text-sm" />
            </div>
          )}
          {ex.rule && <p className="mt-1 text-sm text-[var(--color-muted)]">ℹ️ {ex.rule}</p>}
          {!correct && (
            <div className="mt-2 flex flex-wrap gap-2">
              <button onClick={() => selfGradeOverride()} className="chip">Я был прав, засчитать</button>
              <button onClick={logError} disabled={logged} className="chip">{logged ? '✓ В журнале' : '＋ В журнал ошибок'}</button>
            </div>
          )}
        </div>
      )}
      ...
      {checked && ex.type === 'freeform' && (
        <div className="animate-pop mt-4 rounded-2xl bg-[var(--color-surface2)] p-3">
          {ex.sample && (
            <p className="text-sm"><span className="text-[var(--color-muted)]">Образец: </span><b>{ex.sample}</b></p>
          )}
          <p className="mt-2 text-sm text-[var(--color-muted)]">Как ты справился?</p>
          <div className="mt-2 flex gap-2">
            <button onClick={() => selfGrade(true)} className="btn btn-soft flex-1">😀 Хорошо</button>
            <button onClick={() => selfGrade(false)} className="btn btn-soft flex-1">🤔 Ошибся</button>
          </div>
        </div>
      )}
  ...
  function selfGradeOverride() {
    setCorrect(true);
    onResult(true, given || selected || built.join(' '));
  }
}
