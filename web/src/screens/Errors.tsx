  ...
  return (
    <div>
      <Header back title="Журнал ошибок 🐞" subtitle="как баг-трекер: дотренировать именно это"
        right={<button onClick={() => setAdding((a) => !a)} className="chip !bg-[var(--color-primary)] !text-[#160f33] !font-bold">＋</button>} />
      ...
      {loading || !data ? <Spinner /> : !data.length ? (
        <EmptyState emoji={tab === 'open' ? '🎯' : '🏆'} title={tab === 'open' ? 'Чисто!' : 'Пока пусто'}
            ...
            <div key={e.id} className="card !p-3.5">
              <div className="flex items-start justify-between gap-2">
                <div className="min-w-0 flex-1">
                  ...
                  {e.rule && <p className="mt-1 text-xs text-[var(--color-muted)]">{e.rule}</p>}
                ...
                <div className="flex shrink-0 flex-col gap-1.5">
                  {tab === 'open' ? (
                    <button onClick={() => api.updateError(e.id, { status: 'fixed' }).then(refetch)} className="grid h-8 w-8 place-items-center rounded-full bg-[color-mix(in_srgb,var(--color-success)_20%,transparent)] text-[var(--color-success)]">✓</button>
                  ) : (
                    <button onClick={() => api.updateError(e.id, { status: 'open' }).then(refetch)} className="grid h-8 w-8 place-items-center rounded-full bg-[var(--color-surface2)]">↩</button>
                  )}
                  <button onClick={() => api.deleteError(e.id).then(refetch)} className="grid h-8 w-8 place-items-center rounded-full bg-[var(--color-surface2)] text-[var(--color-muted)]">🗑</button>
                </div>
              </div>
            </div>
          ))}
