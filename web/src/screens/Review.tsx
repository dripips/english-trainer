...
export function Review() {
  ...
  if (!queue.length) {
    return (
      <div>
        ...
        <EmptyState emoji="🎉" title="Всё повторено!" hint="На сегодня карточек нет. Добавь новые слова из словаря или загляни позже."
  ...
  async function rate(r: Rating) {
    try { await api.srsReview(w.id, r); } catch { /* offline: still advance */ }
    setDone((d) => d + 1);
    setShowBack(false);
    setIdx((x) => x + 1);
  }
  ...
  return (
    <div>
      ...
      <div onClick={flip} className="card animate-pop min-h-[16rem] cursor-pointer select-none">
        <div className="flex flex-col items-center justify-center gap-2 py-6 text-center">
          ...
          {showBack ? (
            <div className="animate-slideup mt-4 w-full border-t border-[var(--color-border)] pt-4">
              <div className="display text-2xl font-bold text-[var(--color-mint)]">{w.ru}</div>
              {w.exampleEn && (
            ...
            <p className="mt-6 text-sm text-[var(--color-muted)]">👆 нажми, чтобы увидеть перевод</p>
          )}
        </div>
      </div>
      ...
      <div className="mt-4">
        {showBack ? (
          <div className="grid grid-cols-4 gap-2">
            {RATINGS.map((r) => (
              <button key={r.key} onClick={() => rate(r.key)} className="btn flex-col !gap-0 !py-3 !text-sm" style={{ background: `color-mix(in srgb, ${r.color} 20%, var(--color-surface))`, color: r.color }}>
                {r.label}
              </button>
            ))}
          </div>
